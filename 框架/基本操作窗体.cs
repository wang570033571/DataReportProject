...
namespace 框架
{
    public partial class 基本操作窗体 : 基本窗体
    {
        ...
        {
            InitializeComponent();
            this.Load += new EventHandler(基本操作窗体_Load);
            gridView.RowClick += new DevExpress.XtraGrid.Views.Grid.RowClickEventHandler(gridView_RowClick);
        }

        void gridView_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (this.tabControl明细.Visible == false) return;

            if (gridView.FocusedRowHandle >= 0)
            {
                DataRow dr = gridView.GetFocusedDataRow();
                foreach (Control c in this.tabPage基本信息.Controls)
                {
                    if (c is TextBox)
                    {
                        if (dr.Table.Columns.Contains(c.Tag.ToString()))
        ...
        public virtual void 刷新其他明细() { }

        void 基本操作窗体_Load(object sender, EventArgs e)
        {
            初始化按钮();
            SQL.设置查询控件(panel查询);
            数据窗口.初始化(this, cmData);
            txt记录数.TextChanged += new EventHandler(txt记录数_TextChanged);
            if (this.tabControl明细.Visible == true)
                加载基本信息控件();
        }
        ...
        private void 加载基本信息控件()
        {
            ...
            foreach (DevExpress.XtraGrid.Columns.GridColumn gc in gridView.Columns)
            {
                ...
                lblTitle.Text = gc.Caption;
                lblTitle.Top = (columnIndex % 4 == 0 ? (int)(columnIndex / 4) - 1 : (int)(columnIndex / 4)) * 24 + 14;
                lblTitle.Left = columnIndex % 4 == 1 ? 20 : columnIndex % 4 == 2 ? 270 : columnIndex % 4 == 3 ? 520 : 770;
                lblTitle.Width = 80;
                lblTitle.BackColor = Color.Transparent;
                this.tabPage基本信息.Controls.Add(lblTitle);

                TextBox txtContent = new TextBox();
                txtContent.Top = (columnIndex % 4 == 0 ? (int)(columnIndex / 4) - 1 : (int)(columnIndex / 4)) * 24 + 10;
                txtContent.Left = columnIndex % 4 == 1 ? 100 : columnIndex % 4 == 2 ? 350 : columnIndex % 4 == 3 ? 600 : 850;
                txtContent.Width = 150;
                txtContent.Tag = gc.FieldName;
                ...
                this.tabPage基本信息.Controls.Add(txtContent);
